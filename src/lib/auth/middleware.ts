...
// 보호할 경로 설정
const matchersForAuth = ['/dashboard/*', '/myaccount/*', '/settings/*', '...']; // 인증이 필요한 경로
const matchersForSignIn = ['/signup/*', '/signin/*']; // 로그인 후 접근 X 경로
...
export async function middleware(request: NextRequest) {
  ...
  if (isMatch(request.nextUrl.pathname, matchersForAuth)) {
    return (await getSession())
      ...
      : NextResponse.redirect(new URL('/auth/signin', request.url)); // 없으면 로그인 페이지로 이동
  }
